...
-   all &mdash; Use global locking for all DDL operations.
...
For backwards compatibility, `bdr.ddl_locking` supports aliases. 
...
Track apply statistics for each subscription with [`bdr.stat_subscription`](catalogs-visible.mdx#bdrstat_subscription).

The default value is `on`.

### `bdr.track_relation_apply`

Track apply statistics for each relation with [`bdr.stat_relation`](catalogs-visible.mdx#bdrstat_relation).

The default value is `off`.

### `bdr.track_apply_lock_timing`

Track lock timing when tracking statistics for relations with [`bdr.stat_relation`](catalogs-visible.mdx#bdrstat_relation).
...
use of logical change records (LCRs) for the subscription. By default, this
setting is false. When this setting is true on a downstream node, the node
...
Controls whether TCP keepalives are used. The default value is 1, meaning on. If
...
message to the server. A value of zero uses the system default. This parameter
...
that isn't acknowledged by the server. A value of zero uses the system default.
...
unacknowledged before a connection is forcibly closed. A value of zero uses the
...
being sent to the global (top-level) group. It defaults to 6 seconds (6s).

### `bdr.raft_local_election_timeout`

To account for network failures, the Raft consensus protocol implements timeouts
for elections and requests. This value is used when a request is
being sent to the sub-group. It defaults to 3 seconds (3s).
...
as appropriate. This behavior can be overriden by setting
this variable. The setting of `bdr.raft_response_timeout` should be less than
either of the election timeout values. Set this variable to -1 and the override
is disabled. It defaults to -1.
