...
3. Configure Beacon Agent and database connections.
    ...
    Next, configure Beacon Agent by setting the access key (the one you obtained while [creating a machine user]](create_machine_user)) and project ID, and specify the Beacon config directory for storing the configuration file created. Use the following commands:
